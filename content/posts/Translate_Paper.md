...
* **BLEU**：基础语义对齐评估。
  * 为什么不用大模型评估？
    * 核心是与人对齐。大模型无法解决“拗口"的问题，有时候大模型给一段话平分很高，但是人读起来比较费解。
...
* **幻觉率评估 (Hallucination Rate Evaluation)**：
  * **大模型评估**
    * 大部分的翻译问题都可以由prompt来解决，幻觉问题更多要从训练数据层面入手
...
* **Chosen 和 Rejected 的分布差异过大（OOD）**：
  * **现象**：例如 Chosen 是 32B 生成的完美答案，而训练模型很弱。
  * **举例**：1.8B 的小模型被迫模仿 32B 的高深逻辑 and 复杂句式，就像小学生模仿博士写论文，虽然语气很像（使用了大量“综上所述”、“显而易见”），但中间的推导逻辑全是错的。
