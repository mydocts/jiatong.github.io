...
*   **Teacher-Student 蒸馏**：利用 SOTA 模型（如 GPT-4o）生成高质量翻译作为“金标准”。
    * *Prompt 技巧*：要求模型“先直译，再意译，最后润色”，或者使用 "Chain of Thought" 让模型解释术语。
*   **多模型采样**：使用 Qwen、DeepSeek 等不同参数量的模型生成候选结果，通过人工或模型（LLM-as-a-Judge）筛选最佳答案。
...
* **Rejected (落败项)**：
  ...
  * 出现了幻觉或逻辑错误的生成结果。
* **特殊数据处理**：
*   **图表数据**：使用占位符 `<image>` 或保留 LaTeX 源码，防止模型尝试“翻译”公式变量。
*   **引用与链接**：使用正则匹配进行保护，防止被错误翻译。
*   **长文档**：采用“滑窗+重叠”切分，防止上下文丢失。
...
#### 案例 1：术语一致性幻觉 (Terminology Inconsistency)
...
*   **Chosen 和 Rejected 的分布差异过大（OOD）**：
    * **现象**：例如 Chosen 是 GPT-4 生成的完美答案，而训练模型很弱。
    * **举例**：1.8B 的小模型被迫模仿 GPT-4 的高深逻辑 and 复杂句式，就像小学生模仿博士写论文，虽然语气很像（使用了大量“综上所述”、“显而易见”），但中间的推导逻辑全是错的。
    * **后果**：模型在 SFT 阶段没见过这么高质量的数据，DPO 强行拉近模型与 GPT-4 的分布，导致模型“不懂装懂”，产生逻辑幻觉。
