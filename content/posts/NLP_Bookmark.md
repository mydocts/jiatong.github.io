...
1. **上下文窗口限制**：长链推理会超出模型的最大上下文长度。
2. **噪声累积**：历史推理步骤过多时，模型难以聚焦于关键信息（"Lost in the Middle" 现象）。
...
![Bookmark 训练流程](NLP_Bookmark/training.png)
...
* **奖励函数设计**：
  ...
  * **格式奖励 ($w_{fmt}=0.1$)**：是否正确使用了 `<substep>` 等标签，权重较低。
